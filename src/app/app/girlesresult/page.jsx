...
export default function BoysProgramsWithResults() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (latestResult) {
      ...
      // Insert new row
      const { error } = await supabase.from("girlesresult").insert([
        {
  ...
  const updateTeamTotals = async () => {
    ...
    const { data: girlesresult, error } = await supabase
      .from("girlesresult")
      .select("*");
    
    if (!error && girlesresult) {
      const totals = {
        'ഖുസയ': 0,
        'കിനാന': 0
      };
      
      girlesresult.forEach(result => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {filteredPrograms.map((program) => (
              <div
                ...
                className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-6 cursor-pointer hover:shadow-2xl transition hover:bg-white/15"
              >
